    ...
    class Container
      ...
      def setup()
        ...
        Daemon::Admin.ssh_keys_priv().each do |keyfile|
          Lib::Shell.run("cp #{keyfile} #{rootfspath}/#{SSH_KEY_PREFIX}#{File.basename(keyfile)}") \
            unless File.exists?(File.join(rootfspath,keyfile))
        end
        ...
        if File.exists?(authfile)
          ...
          Daemon::Admin.ssh_keys_pub().each do |keyfile|
            ...
            Lib::Shell.run("echo '#{key}' >> #{authfile}") unless authkeys.include?(key)
          end
        else
          Daemon::Admin.ssh_keys_pub().each do |keyfile|
            Lib::Shell.run("echo '#{IO.read(keyfile).chomp}' >> #{authfile}")
          end
        end
