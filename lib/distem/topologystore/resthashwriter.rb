...
module Distem
  module TopologyStore
    ...
    class RESTHashWriter < HashWriter
      ...
      # Hack to call superclass method an explicit way
      def send_super(meth, *args, &blk)
        # hide current method
        tmp = "_tmp#{@@tmpcount}".to_sym
        @@tmpcount += 1
        self.class.superclass.send(:alias_method, tmp, meth)
        send(tmp, *args, &blk)
        self.class.send(:remove_method, tmp)
        #if self.class.instance_methods(false).include? meth.to_s
        #  self.class.send(:alias_method, :"_tmp#{@@count}", meth)
        #  @@count += 1
      ...
      # See the visit_vplatform documentation
      def visit_cpu(cpu)
          Lib::Shell.run("echo CPU #{cpu.class.name}")
        ret = super(cpu)
      ...
      # See the visit_vplatform documentation
      def visit_vcpu(vcpu)
          Lib::Shell.run("echo VCPU #{vcpu.class.name}")
        ret = super(vcpu)
